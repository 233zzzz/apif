    ...
    for name in names:  #python 字符串连接啊
        ...
        for i in r:
            ...
            if len(result) < 4 and len(result) > 1:
                p = 2
                nodes.append({"name": result, "type": p,"credit_no":idss,"direction":"up"})
                links.append({"source":result,"target":name,"relation":"hold"})
                a.append(result)
            elif len(result) >= 4:
                p = 1
                nodes.append({"name": result, "type": p, "credit_no": idss, "direction": "up"})
    ...
    for name in names:  # python 字符串连接啊
        ...
        for i2 in r2:
            ...
            if len(result2) < 4 and len(result2) > 1:
                p2 = 2
                nodes.append({"name": result2, "type": p2, "credit_no": idss, "direction": "down"})
                links.append({"source": name, "target": result2, "relation": "hold"})
